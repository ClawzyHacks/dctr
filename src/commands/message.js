...
class CommandMessage {
	...
	async run() { // eslint-disable-line complexity
		...
		if(this.command.guildOnly && !this.message.guild) {
			...
			return this.reply(`\`${this.command.name}\` komutu sunucularda kullanılabilir!`);
		}
		...
		try {
			this.client.emit('debug', `${this.command.groupID}:${this.command.memberName} komutu ${this.message.guild ? `${this.message.guild.name} (${this.message.guild.id}) sunucusunda` : "DM kanalında"} ${this.message.author.tag} (${this.message.author.id}) tarafından çalıştırıldı.`)
			...
			if(err instanceof FriendlyError) {
				...
				let ownerList = owners.map((usr, i) => {
					const or = i === owners.length - 1 && owners.length > 1 ? 'ya da ' : '';
				...
				return this.reply(stripIndents`
					...
					Lütfen bildirin: https://discord.gg/4FVYmpb
				`);
			}
		}
	}
	...
	respond({ type = 'reply', content, options, lang, fromEdit = false }) {
		...
		switch(type) {
			...
			case 'code':
				...
				return this.editCurrentResponse(channelIDOrDM(this.message.channel), { type, content, options });
			default:
				throw new RangeError(`Geçersiz! "${type}".`);
		}
	}
